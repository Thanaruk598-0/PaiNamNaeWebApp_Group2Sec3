/**
 * Report API Testing — Incident Report System + Payment Evidence (Bonus)
 * ทดสอบ API endpoints ระบบแจ้งเหตุ + cardBonus Payment Evidence
 *
 * ใช้ Jest + Supertest
 * รัน: cd backend && npx jest ../test/sprint3/API Testing/report.api.test.js --forceExit --detectOpenHandles
 * ⚠️ ต้องเปิด backend server (npm run dev) + seed-paid-payment.js ก่อนรัน
 */
...
beforeAll(async () => {
    ...
    userToken = await loginAs('user01@test.com', '12345678');
    adminToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImNtbWF0Z2l5MjAwMDBkZWl2dWN2ZjZwYjciLCJyb2xlIjoiQURNSU4iLCJpYXQiOjE3NzI1NjA0Mzl9.kyY5AVzo4VOmdn9rKrQ6zaRLgtGwkc0bvKrrrFZZskY";
...
describe('GET /api/reports/admin — Admin ดูรายงานทั้งหมด', () => {
    ...
    test('TC-API-12: กรองตาม incidentType = BEHAVIOR', async () => {
        const res = await request(BASE_URL)
...
// ══════════════════════════════════════
// BONUS CARD: Payment Evidence Confirmation  
// AS a user, I want an evidence to confirm that the payment is succesful.
// Tests payment APIs + PDF evidence downloads
// ══════════════════════════════════════

describe('BONUS: Payment Evidence Confirmation (Card Bonus)', () => {
    let paymentId = '';

    beforeAll(async () => {
        // Fetch user's payments to get PAID payment ID dynamically
        try {
            const res = await request(BASE_URL)
                .get('/api/payments/me')
                .set('Authorization', `Bearer ${userToken}`)
            ...
            if (res.body.success && Array.isArray(res.body.data) && res.body.data.length > 0) {
                const paidPayment = res.body.data.find(p => p.status === 'PAID');
                if (paidPayment) {
                    paymentId = paidPayment.id;
                    console.log('  → BONUS: PAID payment ID:', paymentId);
                } else {
                    console.log('  → BONUS: No PAID payment. Run: cd code/backend && node seed-paid-payment.js');
                }
            } else {
                console.log('  → BONUS: No payments found or server error');
            }
        } catch (error) {
            console.log('  → BONUS: Server may not be running (start npm run dev)');
        }
    }, 20000);

    test('TC-BONUS-01: GET /api/payments/me — User list payments (expect PAID)', async () => {
        const res = await request(BASE_URL)
            .get('/api/payments/me')
            .set('Authorization', `Bearer ${userToken}`);
        ...
        expect(res.body.success).toBe(true);
        expect(Array.isArray(res.body.data)).toBe(true);
        console.log(`  → BONUS: User has ${res.body.data.length} payments`);
    });

    test('TC-BONUS-02: GET /api/payments/:id — Get PAID payment details (success confirmation)', async () => {
        if (!paymentId) {
            console.log('  → BONUS: Skip - no PAID payment');
            return;
        }
        ...
        console.log(`  → BONUS: PAID amount: ${res.body.data.amount}`);
    });

    test('TC-BONUS-03: GET /api/payments/:id/documents — Evidence documents (receipt + tax invoice)', async () => {
        if (!paymentId) {
            console.log('  → BONUS: Skip - no PAID payment');
            return;
        }
        ...
        const res = await request(BASE_URL)
            .get(`/api/payments/${paymentId}/documents`)
            .set('Authorization', `Bearer ${userToken}`);
        ...
        expect(res.body.data.some(d => d.type === 'RECEIPT_VOUCHER')).toBeTruthy();
        expect(res.body.data.some(d => d.type === 'SHORT_TAX_INVOICE')).toBeTruthy();
        console.log('  → BONUS: Evidence PDFs ready');
    });

    test('TC-BONUS-04: GET /api/payments/:id/receipt-voucher — Download receipt PDF evidence', async () => {
        if (!paymentId) {
            console.log('  → BONUS: Skip - no PAID payment');
            return;
        }
        ...
        const res = await request(BASE_URL)
            .get(`/api/payments/${paymentId}/receipt-voucher`)
            .set('Authorization', `Bearer ${userToken}`);

        expect(res.status).toBe(200);
        expect(res.headers['content-type']).toMatch(/pdf|application\/pdf/i);
        console.log('  → BONUS: Receipt voucher PDF downloaded');
    });

    test('TC-BONUS-05: GET /api/payments/me without token → Unauthorized', async () => {
        const res = await request(BASE_URL)
            .get('/api/payments/me');
