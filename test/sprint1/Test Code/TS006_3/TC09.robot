...
Update Report Status To InProgress
    ...
    Open Report Detail
    Update Status    REJECTED    ข้อมูลที่แจ้งมาเป็นเท็จ
...
Open Report Detail
    ...
    Capture Page Screenshot    rejected_detail_page_loaded.png
...
Update Status
    ...
    Capture Page Screenshot    rejected_status_selected.png
    ...
    Capture Page Screenshot    rejected_note_entered.png
    ...
    Capture Page Screenshot    rejected_save_clicked.png

    Sleep    2s

    Reload Page
    Wait Until Element Is Visible    xpath=//select    10s
    Capture Page Screenshot    rejected_page_reloaded.png

    ${selected_label}=    Get Selected List Label    xpath=//select
    Should Contain    ${selected_label}    ปฏิเสธ
    Capture Page Screenshot    rejected_status_verified.png

    ${saved_note}=    Get Value    xpath=//textarea[contains(@placeholder,'หมายเหตุ')]
    Should Be Equal    ${saved_note}    ${note}
    Capture Page Screenshot    rejected_note_verified.png
