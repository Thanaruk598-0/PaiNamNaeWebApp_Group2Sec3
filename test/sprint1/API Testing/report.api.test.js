...
beforeAll(async () => {
    ...
    userToken = await loginAs('pp@gmail.com', '12345678');

    // Login as admin
    adminToken = await loginAs('pooh@gmail.com', '12345678');

    console.log('User token:', userToken ? '✅ ได้รับ token' : '❌ ไม่ได้ token');
    console.log('Admin token:', adminToken ? '✅ ได้รับ token' : '❌ ไม่ได้ token');
}, 15000);
...
describe('POST /api/reports — User สร้างรายงานใหม่', () => {
    ...
    test('TC-API-01: สร้างรายงานสำเร็จ (ครบทุก field)', async () => {
        ...
        expect(res.body.data.incidentType).toBe('DRIVER');
...
describe('GET /api/reports/admin — Admin ดูรายงานทั้งหมด', () => {
    ...
    test('TC-API-12: กรองตาม incidentType = DRIVER', async () => {
        const res = await request(BASE_URL)
            .get('/api/reports/admin?incidentType=DRIVER')
            .set('Authorization', `Bearer ${adminToken}`);

        expect(res.status).toBe(200);
        if (res.body.data && res.body.data.length > 0) {
            res.body.data.forEach(report => {
                expect(report.incidentType).toBe('DRIVER');
            });
        }
    });
