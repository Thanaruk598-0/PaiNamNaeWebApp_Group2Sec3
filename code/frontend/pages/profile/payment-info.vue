            ...
            <div class="p-6 bg-white border border-gray-300 shadow rounded-xl">
              <div class="flex items-center justify-between gap-4">
                <div class="min-w-0">
                  ...
                  <p class="mt-1 text-sm text-gray-500">
                    ระบุเบอร์โทร/เลขบัตร/เลขประจำตัวผู้เสียภาษี
                  </p>
                </div>
              </div>
              ...
              <div class="mt-4 pt-4 border-t border-gray-200">
                <div class="flex items-center justify-between gap-3">
                  ...
                  <label
                    class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-blue-700 bg-blue-50 rounded-md hover:bg-blue-100 cursor-pointer"
                  >
                    เลือกไฟล์
                    <input
                      class="hidden"
                ...
                <p v-if="qrError" class="mt-2 text-xs text-red-600">
                  {{ qrError }}
                </p>
                ...
                >
                  <img
                    :src="qrPreviewUrl"
                    class="w-full h-44 object-contain bg-white"
                    alt="promptpay qr preview"
                  />
                </div>
                <div v-else-if="promptPayQrUrl" class="mt-3">
                  <div
                    v-if="!isPdfUrl(promptPayQrUrl)"
                ...
                <div class="mt-3 flex items-center gap-3">
                  <button
                    type="button"
                    @click="uploadPromptPayQr"
                    :disabled="isUploadingQr || !qrFile"
                ...
                <ul v-else class="space-y-3">
                  ...
                  >
                    <div class="flex items-center gap-3 min-w-0">
                      <div
                        class="flex items-center justify-center w-10 h-10 rounded-full overflow-hidden border border-gray-200"
    ...
    >
      ...
      <div class="relative w-full max-w-xl bg-white rounded-2xl shadow-xl">
            ...
            <div class="relative">
              ...
              >
                <div class="max-h-64 overflow-y-auto">
                  ...
                  >
                    <div
                      class="flex items-center justify-center w-8 h-8 rounded-full overflow-hidden border border-gray-200"
        ...
        <div class="grid grid-cols-2 border-t border-gray-200">
          <button
            type="button"
            class="px-6 py-4 text-gray-700 hover:bg-gray-50"
            @click="closeModal"
          >
            ยกเลิก
          </button>
...
const fetchPaymentInfo = async () => {
  ...
  try {
    ...
    bankAccounts.value = Array.isArray(data?.bankAccounts)
      ? data.bankAccounts
      : [];
  } catch (err) {
    toast.error(
      "เกิดข้อผิดพลาด",
      err.data?.message || "ไม่สามารถดึงข้อมูลการรับเงินได้",
    );
  } finally {
...
const savePromptPay = async () => {
  ...
  try {
    ...
    toast.error(
      "เกิดข้อผิดพลาด",
      err.data?.message || "ไม่สามารถบันทึก PromptPay ได้",
    );
  } finally {
...
const uploadPromptPayQr = async () => {
  ...
  try {
    ...
    toast.error(
      "เกิดข้อผิดพลาด",
      err?.statusMessage || err?.data?.message || "ไม่สามารถอัปโหลดไฟล์ QR ได้",
    );
  } finally {
...
const submitBankAccount = async () => {
  ...
  try {
    ...
    toast.error(
      "เกิดข้อผิดพลาด",
      err.data?.message || "ไม่สามารถบันทึกบัญชีธนาคารได้",
    );
  } finally {
...
const deleteAccount = async (acc) => {
  ...
  try {
    await $api(`/payment-methods/bank-accounts/${acc.id}`, {
      method: "DELETE",
    });
