<template>
  <Teleport to="body">
    <Transition name="modal-fade">
      <div
        v-if="modelValue"
        class="fixed inset-0 z-[70] flex items-start sm:items-center justify-center bg-black/40 backdrop-blur-sm p-2 sm:p-4"
        @click.self="close"
      >
        <div
          class="w-full max-w-[760px] bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[94vh] flex flex-col"
        >
          <!-- Header -->
          <div
            class="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 flex items-start justify-between shrink-0"
          >
            <div>
          ...
          <!-- Body -->
          <div class="px-4 sm:px-6 py-4 sm:py-5 overflow-y-auto min-h-0">
            <!-- Summary card -->
            <div
              class="p-5 bg-gradient-to-br from-slate-50 to-white rounded-2xl border border-slate-100"
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <p class="text-sm font-semibold text-slate-500">ผู้รับเงิน</p>
                  <p class="mt-0.5 text-base font-bold text-slate-800 truncate">
                    {{ receiverLabel }}
                  </p>
                </div>
                <div class="text-right shrink-0">
                  ...
                  <p
                    class="mt-0.5 text-2xl font-extrabold text-blue-600 leading-none"
                  >
                    {{ amountLabel }}
              ...
              <div class="mt-3 flex items-center gap-2 text-sm text-slate-500">
                <span
                  class="inline-flex items-center px-2 py-1 rounded-full bg-white border border-slate-100"
                >
                  ...
                  <div class="flex items-center justify-between">
                    <div>
                      ...
                      <p class="text-sm text-gray-600 leading-snug">
                        สแกน/ดู QR หรือโอนเข้าบัญชี แล้วอัปโหลดสลิป
                      </p>
                    </div>
                    <span
                      class="text-sm font-bold text-blue-700 bg-blue-50 px-2.5 py-1 rounded-full"
                      >แนะนำ</span
                    >
                  </div>
                </button>
              ...
              <div v-if="method === 'BANK_TRANSFER'" class="mt-4">
                <div class="flex items-center justify-between">
                  <p class="text-base font-bold text-slate-800">
                    ช่องทางรับเงินของคนขับ
                  </p>
                  <p class="text-sm text-slate-500">เลือกโอนช่องทางใดก็ได้</p>
                </div>
                <div v-if="isLoadingMethods" class="mt-2 text-sm text-gray-500">
                  กำลังโหลด…
                </div>
                <div v-else class="mt-3 space-y-3">
                    ...
                    <div class="flex items-start justify-between gap-3">
                      <div class="min-w-0">
                        ...
                        <div class="flex items-center gap-2">
                          <p
                            class="mt-1 font-extrabold text-gray-900 text-lg leading-none"
                          >
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                          ...
                          <span class="font-semibold text-slate-700">{{
                            driverMethods.ownerName || receiverLabel
                          }}</span>
                        </p>
                      </div>
                      <div
                        class="shrink-0 text-sm font-bold text-gray-500 bg-slate-50 border border-slate-100 px-2.5 py-1 rounded-full"
                      >
                        PROMPTPAY
                      </div>
                    </div>
                  ...
                  >
                    <div class="flex items-center justify-between gap-3">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          @click="
                            openFullscreenPreview(driverMethods.promptPayQrUrl)
                          "
                    ...
                    <div
                      v-if="!isPdf(driverMethods.promptPayQrUrl)"
                      class="mt-3 rounded-xl overflow-hidden border border-gray-100 bg-white"
                    >
                      <img
                        :src="driverMethods.promptPayQrUrl"
                        class="w-full h-52 sm:h-60 object-contain bg-white"
                        alt="promptpay qr"
                      />
                    </div>
                    <button
                  ...
                  >
                    ...
                    <div class="min-w-0 flex-1">
                      <p
                        class="text-base font-bold text-gray-900 truncate leading-tight"
                      >
                        {{ bankLabel(acc.bankCode, acc.customBankName) }}
                      </p>
                      <p
                        class="mt-1 text-sm text-gray-500 flex flex-wrap items-center gap-x-2 gap-y-1"
                      >
                        <span
                          class="flex items-center gap-1.5 font-bold text-slate-700 tracking-wide"
                        >
                          <span class="break-all">{{ acc.accountNumber }}</span>
                          <button
                            type="button"
                            @click="
                              copyToClipboard(acc.accountNumber, 'เลขบัญชี')
                            "
                        ...
                        <span
                          v-if="driverMethods.ownerName"
                          class="text-gray-300"
                          >•</span
                        >
                        <span v-if="driverMethods.ownerName" class="truncate">{{
                          driverMethods.ownerName
                        }}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    v-if="
                      !driverMethods.promptPayId &&
                      driverMethods.bankAccounts.length === 0
                    "
            ...
            <div v-else-if="step === 2" class="mt-5">
              ...
              <p class="mt-1 text-sm text-gray-500">
                อัปโหลดสลิปหรือภาพหน้าจอยืนยันการโอนเงิน
              </p>
              ...
              <div class="mt-4">
                ...
                >
                  ...
                  />
                  <div
                    class="mx-auto w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-sm border border-gray-100"
                  >
                    <svg
                ...
                <p v-if="slipError" class="mt-2 text-sm text-red-600">
                  {{ slipError }}
                </p>

                <div
                  v-if="slipPreviewUrl"
                  class="mt-4 rounded-2xl overflow-hidden border border-gray-200"
                >
                  <img
                    :src="slipPreviewUrl"
            ...
            <div v-else class="mt-5">
              <div
                class="flex items-start gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100"
              >
                <div
              ...
              <div v-if="method === 'BANK_TRANSFER'" class="mt-4">
                ...
                <div
                  v-if="slipPreviewUrl"
                  class="mt-2 rounded-2xl overflow-hidden border border-gray-200"
                >
                  <img
                    :src="slipPreviewUrl"
              ...
              <div
                class="mt-4 text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-xl p-3"
              >
                เมื่อส่งแล้วระบบจะให้คนขับตรวจสอบข้อมูลให้ถูกต้องก่อนกด
                “ยืนยันการชำระเงิน”
              </div>
            </div>
          </div>
          ...
          <!-- Footer buttons -->
          <div
            class="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100 flex items-center gap-3 shrink-0 bg-white"
          >
            <button
              type="button"
        ...
        <div
          v-if="fullscreenPreviewOpen"
          class="fixed inset-0 z-[80] bg-black/85 backdrop-blur-sm p-3 sm:p-6"
          @click.self="closeFullscreenPreview"
        >
          <div class="relative w-full h-full flex items-center justify-center">
            <button
              type="button"
              class="absolute top-2 right-2 sm:top-4 sm:right-4 w-10 h-10 rounded-full bg-white/90 hover:bg-white text-gray-700 flex items-center justify-center"
              @click="closeFullscreenPreview"
              aria-label="ปิดหน้าขยาย"
            >
              <svg
                class="w-5 h-5"
                viewBox="0 0 24 24"
...
const fullscreenPreviewIsPdf = ref(false);

const logoFailed = reactive({});

const receiverLabel = computed(() =>
  props.trip?.driver?.name ? props.trip.driver.name : "คนขับ",
);
const amountLabel = computed(() =>
  Number(props.trip?.price || 0).toLocaleString(),
);
...
const canNext = computed(() => {
  ...
  if (step.value === 2)
    return method.value === "CASH" ? true : !!slipFile.value;
  return true;
});
...
async function fetchDriverMethods() {
  ...
  try {
    ...
    driverMethods.value = data?.driverPaymentMethods || {
      ownerName: null,
      promptPayId: null,
      promptPayQrUrl: null,
      bankAccounts: [],
...
function openFullscreenPreview(url) {
  ...
  fullscreenPreviewUrl.value = normalizedUrl;
  fullscreenPreviewIsPdf.value = isPdf(normalizedUrl);
...
function closeFullscreenPreview() {
  ...
  fullscreenPreviewIsPdf.value = false;
}
...
async function submit() {
  ...
  try {
    ...
    if (method.value === "CASH") {
      ...
      await $fetch(
        `${useRuntimeConfig().public.apiBase}payments/booking/${bookingId}/slip`,
        {
          method: "POST",
          body: formData,
          headers: {
    ...
    toast.error(
      "เกิดข้อผิดพลาด",
      err?.statusMessage || err?.data?.message || "ไม่สามารถส่งข้อมูลได้",
    );
  } finally {
